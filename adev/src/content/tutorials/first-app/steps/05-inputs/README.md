...
In this lesson, you'll assign `input` functionality in the `HousingLocation` component which will enable you to customize the data displayed in the component.
...
<docs-step title="Import the Input signal">
This step imports the `input` signal into the class.
...
1.  In the same file, add a property named `housingLocation` and initialize it using `input.required()` with the type `HousingLocationInfo`. To set the type, use a generic parameter, by writing     <code>&lt;HousingLocationInfo&gt;</code> immediately after <code>.required</code>:
...
SUMMARY: In this lesson, you created a new property with the `input` signal. You also used the `.required` method, which tells the Typescript compiler that the property won't be `null` or `undefined`.
